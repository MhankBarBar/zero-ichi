...
class _ConfigProxy:
    """Proxy that reads from runtime_config on each access."""

    @property
    def BOT_NAME(self) -> str:
        return _get_config().bot_name
    ...
    def LOGIN_METHOD(self) -> str:
        return _get_config().login_method

    @property
    def PHONE_NUMBER(self) -> str:
        return _get_config().phone_number

    @property
    def OWNER_JID(self) -> str:
        return _get_config().get_owner_jid()

    @property
    def IGNORE_SELF_MESSAGES(self) -> bool:
        return _get_config().get_nested("bot", "ignore_self_messages", default=True)

    @property
    def LOG_MESSAGES(self) -> bool:
        return _get_config().get_nested("logging", "log_messages", default=True)

    @property
    def VERBOSE_LOGGING(self) -> bool:
        return _get_config().get_nested("logging", "verbose", default=False)

    @property
    def LOG_LEVEL(self) -> str:
        return _get_config().get_nested("logging", "level", default="INFO")

    @property
    def FILE_LOGGING(self) -> bool:
        return _get_config().get_nested("logging", "file_logging", default=True)

    @property
    def AUTO_READ(self) -> bool:
        return _get_config().get_nested("bot", "auto_read", default=True)

    @property
    def AUTO_RELOAD(self) -> bool:
        return _get_config().get_nested("bot", "auto_reload", default=True)


_proxy = _ConfigProxy()

BOT_NAME = property(lambda self: _proxy.BOT_NAME)
PREFIX = property(lambda self: _proxy.PREFIX)
LOGIN_METHOD = property(lambda self: _proxy.LOGIN_METHOD)
PHONE_NUMBER = property(lambda self: _proxy.PHONE_NUMBER)
OWNER_JID = property(lambda self: _proxy.OWNER_JID)
IGNORE_SELF_MESSAGES = property(lambda self: _proxy.IGNORE_SELF_MESSAGES)
LOG_MESSAGES = property(lambda self: _proxy.LOG_MESSAGES)
VERBOSE_LOGGING = property(lambda self: _proxy.VERBOSE_LOGGING)
LOG_LEVEL = property(lambda self: _proxy.LOG_LEVEL)
FILE_LOGGING = property(lambda self: _proxy.FILE_LOGGING)
